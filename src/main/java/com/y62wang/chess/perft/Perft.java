...
public class Perft
{
    ...
    private static void perft(Bitboard startingBoard, int targetDepth, int currentDepth, Map<Integer, Long> nodes, Map<String, Integer> roots) {
        if (currentDepth == targetDepth) {
        ...
        for (short possibleMove : startingBoard.legalMoves()) {
            // System.out.println(">> " +Move.moveString(possibleMove) + " " + Move.isEnpassant(possibleMove));
            Bitboard board = startingBoard.makeMove(possibleMove);
            if (currentDepth == 0) {
                board.rootMove = possibleMove;
            } else {
            ...
            perft(board, targetDepth, currentDepth + 1, nodes, roots);
        }
    }
}
